...
function loadGlobalVariables(){
  MAGE_DOMAIN=acadia.harriswebworks.com
  MAGE_WEB_USER=acadia
  MAGE_DB_HOST=localhost
  MAGE_DB_NAME=acadia_live
  MAGE_DB_USER_NAME=wu_acadia
...
function installingStartingServices(){Beng0lT1g3r
...
function readME(){
  echo -e "
  ----------------------------------------------------------------
    Tool Name: WordPress/Drupal SetupTool- Part-1
    Version: v.1.0
...
#PART-B: Preparing the Script @/root/mage/.prepared
prepareTheScript

#PART-C: Installing and Starting the following services
installingStartingServices

#PART-D: Altering the sshd_config Parameters
alterSSHDconfig

#PART-E: Update and upgrade, enable modules and config-manager, wget repos
updateUpgradeEnableModules
